...
module dl_fpga_prj #(
   ...
   parameter MEM_DQS_WIDTH         = 2              // DDR3 DQS信号数量（2个，对应8bit字节使能）
    ...
    output                               tx_disable                    ,
    output  [3:0]                        o_pl_err                      ,        
    output  [1:0]                        cam_init_done                 ,    // 摄像头初始化完成标志 [1]=cam2, [0]=cam1
...
//===============================================================================
// 通道1视频数据选择：使用摄像头1的数据
assign     pclk_in_test    =    cam1_pclk_16bit    ;      // 像素时钟
assign     vs_in_test      =    cam1_vsync_d0      ;      // 场同步信号
assign     de_in_test      =    cam1_href_16bit    ;      // 数据有效信号
// RGB565格式转换：将16bit数据重新排列为RGB565格式 {R[4:0], G[5:0], B[4:0]}
assign     i_rgb565        =    {cam1_data_16bit[4:0],cam1_data_16bit[10:5],cam1_data_16bit[15:11]};

// 通道2视频数据选择：使用摄像头2的数据
assign     pclk_in_test_2  =    cam2_pclk_16bit    ;      // 像素时钟
assign     vs_in_test_2    =    cam2_vsync_d0      ;      // 场同步信号
assign     de_in_test_2    =    cam2_href_16bit    ;      // 数据有效信号
// RGB565格式转换：将16bit数据重新排列为RGB565格式 {R[4:0], G[5:0], B[4:0]}
assign     i_rgb565_2      =    {cam2_data_16bit[4:0],cam2_data_16bit[10:5],cam2_data_16bit[15:11]};

assign     fmc_pclk = cam_fmc_pclk_16bit;
assign     fmc_vs   = cam_fmc_vsync_d0;
assign     fmc_de   = cam_fmc_href_16bit;
assign     fmc_rgb565 = {cam_fmc_data_16bit[4:0],cam_fmc_data_16bit[10:5],cam_fmc_data_16bit[15:11]};
...
img_data_stream_reducer dl_ch0_image_reshape(
    .clk                    (rx_clk                     ),   // 输入：像素时钟
    .rst_n                  (lock && ddr_init_done      ),   // 输入：复位信号（PLL锁定且DDR初始化完成）

    .img_vs                 (vs_wr                      ),   // 输入：场同步信号
    .img_data_valid         (de_wr                      ),   // 输入：数据有效信号
    .img_data               (vout_data_r                ),   // 输入：RGB565格式图像数据
...
(
      ...
      // 通道0   
      .ch0_wframe_pclk             (rx_clk                                    ),
      .ch0_wframe_rst_n            (lock && ddr_init_done                     ),
      .ch0_wframe_vsync            (vs_wr                                ),
