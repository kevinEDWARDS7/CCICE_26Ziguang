...
always @ (posedge chk_clk or negedge i_chk_rstn) begin 
    if(i_chk_rstn==1'b0)
        rxbyte_shft <= 2'd0;
    else if(gt_rx_ctrl[0] == 1'b1 && gt_rx_ctrl[3:1] == 3'b000) begin // K 字符在 Byte0
        rxbyte_shft <= 2'd0; // 无需移位
    end
    else if(gt_rx_ctrl[1] == 1'b1 && (gt_rx_ctrl[0] == 1'b0 && gt_rx_ctrl[3:2] == 2'b00)) begin // K 字符在 Byte1
        rxbyte_shft <= 2'd1; // 移位一个字节
    end
    else if(gt_rx_ctrl[2] == 1'b1 && (gt_rx_ctrl[0] == 1'b0 && gt_rx_ctrl[1] == 1'b0 && gt_rx_ctrl[3] == 1'b0)) begin // K 字符在 Byte2
        rxbyte_shft <= 2'd2; // 移位两个字节
    end
    else if(gt_rx_ctrl[3] == 1'b1 && gt_rx_ctrl[2:0] == 3'b000) begin // K 字符在 Byte3
        rxbyte_shft <= 2'd3; // 移位三个字节
    end
