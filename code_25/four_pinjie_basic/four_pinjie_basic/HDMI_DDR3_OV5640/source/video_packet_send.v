...
assign video_frame_sync = vs_tx_clk_d1 & ~vs_tx_clk_d2;       //VS上升沿
...
wire pclk_rst_pulse;
assign pclk_rst_pulse = vs_pclk_d1 & ~vs_pclk_d2;

//2 级触发器，将 pclk 脉冲同步到 tx_clk 域
reg tx_clk_rst_sync0;
reg tx_clk_rst_sync1;
wire tx_clk_rst_pulse; // 这是将在 tx_clk 域中使用的新复位信号

always@(posedge tx_clk or posedge rst)
begin
    if (rst) begin
        tx_clk_rst_sync0 <= 1'b0;
        tx_clk_rst_sync1 <= 1'b0;
    end
    else begin
        tx_clk_rst_sync0 <= pclk_rst_pulse;
        tx_clk_rst_sync1 <= tx_clk_rst_sync0;
    end
end

reg  pclk_rst_sync0, pclk_rst_sync1, pclk_rst_sync2;
wire pclk_rst_edge = pclk_rst_sync1 & ~pclk_rst_sync2;
...
always @(posedge tx_clk or posedge rst) begin
    if(rst) begin
        ...
        pclk_rst_sync0 <= pclk_rst_pulse; // 从 pclk 域采样
        pclk_rst_sync1 <= pclk_rst_sync0;
