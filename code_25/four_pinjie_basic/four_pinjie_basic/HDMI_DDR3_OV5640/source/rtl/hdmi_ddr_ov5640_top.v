...
wire             sfpin_scale_de;
wire  [23:0]     sfpin_scale_data;
video_scale_process#(
    .PIX_DATA_WIDTH       ( 24 )
)u_video_scale_process_3_sfpin(
    .video_clk            ( rx_clk            ),
    .rst_n                ( rstn_out          ),
    .frame_sync_n         ( ~sfpin_vs_d1      ),
    .video_data_in        ( {{vout_data_r[15:11],3'b0},{vout_data_r[10:5],2'b0},{vout_data_r[4:0],3'b0}}       ), 
    .video_data_valid     ( de_wr             ),
    .video_data_out       ( sfpin_scale_data  ),
    .video_data_out_valid ( sfpin_scale_de    ),
    .video_ready          ( 1'b1              ),
    .video_width_in       ( 1280              ),
    .video_height_in      ( 720               ),
    .video_width_out      ( 640               ),
...
//channel 3 发送请求
cmos_write_req_gen cmos_write_req_gen_m3_sfpin(
	.rst                        (~rstn_out                 ),
	.pclk                       (rx_clk             ),
	.cmos_vsync                 (vs_wr               ),
...
//通道3
assign ch3_write_data = {sfpin_scale_data[23:19],sfpin_scale_data[15:10],sfpin_scale_data[7:3]};
assign ch3_write_en   = sfpin_scale_de;
assign ch3_clk        = rx_clk;
