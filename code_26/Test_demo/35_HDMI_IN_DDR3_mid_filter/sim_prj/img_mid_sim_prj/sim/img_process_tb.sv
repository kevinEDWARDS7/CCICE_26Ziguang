...
begin
	...
	#20
	rst_n		=	1'd1;
	output_file	=	$fopen("D:/2L676demo/img_mid_sim_prj/img_mid_sim_prj/sim/img_process.txt","w");
end
//生成 100MHZ
...
median_filter_3x3 u_median_filter_3x3(
    ...
    .data32      ( matrix32     ),
    .data33      ( matrix33     ),
    .target_data ( mid_filter_data ),
    .vsync_out   ( mid_filter_vs   ),
    .hsync_out   (    ),
    .de_out      ( mid_filter_de      )
);
...
always@(posedge video_clk or negedge rst_n)	begin
	if(!rst_n)
		...
		video_vs_d	<=	mid_filter_vs;
end

assign frame_flag = ~mid_filter_vs & video_vs_d;    //下降沿
...
always@(posedge video_clk or negedge rst_n)	begin
	...
	else if(mid_filter_de)    //写入数据
	begin
		$fdisplay(output_file,"%h\t%h\t%h",mid_filter_data,mid_filter_data,mid_filter_data);    //16进制写入  
	end
end
