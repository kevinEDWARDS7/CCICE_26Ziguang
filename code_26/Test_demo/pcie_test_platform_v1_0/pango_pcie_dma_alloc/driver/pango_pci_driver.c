...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case PCI_MAP_ADDR_CMD:
			...
			dma_info.addr_r.data_buf = dma_alloc_coherent(&op_dev->dev, dma_operation.current_len * 4,
								      &dma_info.addr_r.addr, GFP_KERNEL);
			dma_info.addr_w.data_buf = dma_alloc_coherent(&op_dev->dev, dma_operation.current_len * 4,
								      &dma_info.addr_w.addr, GFP_KERNEL);
			if (!dma_info.addr_r.data_buf || !dma_info.addr_w.data_buf) {
				if (dma_info.addr_r.data_buf) {
					dma_free_coherent(&op_dev->dev, dma_operation.current_len * 4,
		...
		case PCI_UMAP_ADDR_CMD:
			...
			if (dma_info.addr_r.data_buf) {
				dma_free_coherent(&op_dev->dev, dma_operation.current_len * 4,
						  ...
						  dma_info.addr_r.addr - dma_operation.offset_addr);
			}
			if (dma_info.addr_w.data_buf) {
				dma_free_coherent(&op_dev->dev, dma_operation.current_len * 4,
						  dma_info.addr_w.data_buf,
						  dma_info.addr_w.addr - dma_operation.offset_addr);
			}
			dma_info.addr_r.data_buf = NULL;
			dma_info.addr_w.data_buf = NULL;
			spin_unlock(&dma_info.addr_r.lock);
		...
		case PCI_PERFORMANCE_START_CMD:
			...
			performance_config.addr.data_buf = dma_alloc_coherent(&op_dev->dev, DMA_MAX_PACKET_SIZE * 10 * 2,
		...
		case PCI_PERFORMANCE_END_CMD:
			...
			if (copy_to_user((PERFORMANCE_OPERATION __user *)arg, &performance_operation,
					 sizeof(PERFORMANCE_OPERATION))) {
				spin_unlock(&performance_config.addr.lock);
				up(&pci_pango->_sem);
				return -EFAULT;
			}
			dma_free_coherent(&op_dev->dev, DMA_MAX_PACKET_SIZE * 10 * 2,
					  performance_config.addr.data_buf, performance_config.addr.addr);
...
int init_pango_cdev_class(struct class **pango_cdev_class)
{
	...
	*pango_cdev_class = class_create(THIS_MODULE, pci_dev_info._dev_name);
...
#ifndef NDEBUG
	(!IS_ERR(*pango_cdev_class)) ? LOG("class create success.\n") : LOG("class create failed.\n");
#endif
	...
	LOG("create device.\n");
	pdev = device_create(*pango_cdev_class, NULL, pci_dev_info._dev, NULL, "%s", pci_dev_info._dev_name);
	if(IS_ERR(pdev))
	{
		result = PTR_ERR(pdev);
