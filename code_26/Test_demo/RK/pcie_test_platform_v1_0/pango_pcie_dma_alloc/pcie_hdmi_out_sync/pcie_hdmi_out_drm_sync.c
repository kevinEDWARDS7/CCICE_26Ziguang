...
typedef struct {
    ...
    unsigned int readback_polls;
    unsigned int readback_poll_sleep_us;
    unsigned int bootstrap_require_nonzero;
...
typedef struct {
    ...
    unsigned long long readback_poll_reads;
    unsigned long long dma_cmd_retries;
    unsigned long long late_nonzero_lines;
    unsigned long long bootstrap_wait_lines;
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "[--fps 0] [--busy-wait 4000] [--dma-retries 8] [--retry-sleep-us 100] [--max-line-failures 0] [--readback-polls 8] [--readback-poll-sleep-us 50] [--bootstrap-require-nonzero 1] "
...
static int recv_one_line(int fd,
                         ...
                         RxSyncState *rx_state,
                         PipelineStats *stats,
                         int *last_err)
{
    int err = EIO;
    unsigned int attempts = ctx->dma_retries + 1U;
    unsigned int polls = ctx->readback_polls + 1U;

    dma->cmd = PCI_DMA_WRITE_CMD;

    for (unsigned int attempt = 0; attempt < attempts; ++attempt) {
        if (attempt > 0U) {
            ++stats->dma_cmd_retries;
        }
        ...
        if (ioctl(fd, PCI_DMA_WRITE_CMD, dma) != 0) {
            ...
            sleep_us(ctx->retry_sleep_us);
            continue;
        }

        for (volatile unsigned int i = 0; i < ctx->busy_wait_loop; ++i) {
        }
        ...
        for (unsigned int poll = 0; poll < polls; ++poll) {
            ...
            ++stats->readback_poll_reads;
            nonzero = count_nonzero_bytes(dma->data.read_buf, dma->current_len * 4U);

            if (nonzero > 0U) {
                if (poll > 0U) {
                    ++stats->late_nonzero_lines;
                }
                rx_state->stream_locked = 1U;
                if (last_err) {
                    *last_err = 0;
                }
                return 0;
            ...
            } else if (ctx->bootstrap_require_nonzero) {
                ++stats->bootstrap_wait_lines;
            } else {
                if (last_err) {
                    *last_err = 0;
                }
            ...
            sleep_us(ctx->readback_poll_sleep_us);
        }
    ...
    if (!ctx->bootstrap_require_nonzero && !rx_state->stream_locked) {
        if (ioctl(fd, PCI_READ_FROM_KERNEL_CMD, dma) == 0) {
            ++stats->readback_poll_reads;
            if (last_err) {
                *last_err = 0;
                               ...
                               unsigned int expect_lines)
{
    ...
    fprintf(stdout,
            "[debug] t=%.2fs frame=%llu fps=%.2f rx=%.2f MiB dma_ok_lines=%llu dma_fail_lines=%llu all_zero=%llu changed=%llu polls=%llu dma_retry=%llu late_ready=%llu boot_wait=%llu dumped=%llu\n",
            elapsed_total,
            stats->frame_index,
            fps,
            mbps,
...
static int run_pipeline(AppContext *ctx, DrmOutput *drm)
{
    ...
    fprintf(stdout,
            "start: src=%ux%u %s, display=%ux%u, fps_limit=%u, busy_wait=%u, retries=%u, retry_sleep_us=%u, readback_polls=%u, readback_poll_sleep_us=%u, bootstrap_require_nonzero=%u",
            ctx->src_width,
            ctx->src_height,
            src_fmt_name(ctx->src_format),
            drm->mode.hdisplay,
            ...
            ctx->retry_sleep_us,
            ctx->readback_polls,
            ctx->readback_poll_sleep_us,
            ctx->bootstrap_require_nonzero);
    if (ctx->src_format == SRC_FMT_RGB565) {
    ...
    while (!g_stop) {
        ...
        for (unsigned int y = 0; y < yres_use; ++y) {
            ...
            if (recv_one_line(ctx->pcie_fd, &ctx->dma, ctx, &rx_state, &stats, &dma_err) != 0) {
                ++line_fail_streak;
                ++dropped_lines;
...
int main(int argc, char **argv)
{
    ...
    ctx.readback_polls = 8;
    ctx.readback_poll_sleep_us = 50;
    ctx.bootstrap_require_nonzero = 1;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (strcmp(argv[i], "--readback-polls") == 0 && i + 1 < argc) {
            ctx.readback_polls = parse_u32(argv[++i], "readback-polls");
        } else if (strcmp(argv[i], "--readback-poll-sleep-us") == 0 && i + 1 < argc) {
            ctx.readback_poll_sleep_us = parse_u32(argv[++i], "readback-poll-sleep-us");
        } else if (strcmp(argv[i], "--bootstrap-require-nonzero") == 0 && i + 1 < argc) {
            ctx.bootstrap_require_nonzero = parse_u32(argv[++i], "bootstrap-require-nonzero");
