...
- `IMAGE_WIDTH=1920`、`IMAGE_HEIGHT=1080`、`LINE_BYTES=3840`。这与当前 FPGA HDMI 直通 + DDR `FRAME_LEN=259200` 的 1920x1080 RGB565 链路一致。
...
```sh
sudo ./pango_pcie_drm_c --no-display --width 1920 --height 1080 --line-bytes 3840 --frames 1 --dump-lines 8 --dump-frame /tmp/hdmi_pcie_8lines.rgb565
ls -l /tmp/hdmi_pcie_8lines.rgb565
...
- `--width N`，默认 `1920`
- `--height N`，默认 `1080`
- `--line-bytes N`，默认 `3840`
- `--frames N`，默认 `0`，表示 Ctrl+C 前持续运行
- `--delay-loops N`，默认 `4000`
- `--no-display`，不打开 DRM，只做 PCIe probe、DMA 读帧、统计和可选 dump
- `--dump-frame PATH`，保存从 PCIe 读到的原始 RGB565 数据
- `--dump-lines N`，只保存前 N 行；不指定则保存整帧

DRM/KMS 会自动选择 connected connector，优先输入尺寸对应模式；如果没有该模式，选择 preferred mode。显示 buffer 为 XRGB8888 dumb buffer，程序会把 1920x1080 RGB565 近邻缩放到实际 HDMI OUT 模式。
...
- 确认 FPGA 每行输出为 3840 字节，即 240 个 128-bit beat。
- 调整 `--delay-loops`。
...
本工程默认 FPGA 侧 bitstream 已正确完成 HDMI IN/图像输入、RGB565 组织、PCIe endpoint 枚举、DMA 请求响应和每行 3840 字节输出。RK3568 侧无法替代 FPGA 侧时序、帧同步和数据格式保证。
