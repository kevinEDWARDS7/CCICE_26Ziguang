...
- `IMAGE_WIDTH=640`、`IMAGE_HEIGHT=360`、`LINE_BYTES=1280`。这与当前 FPGA reducer + DDR `FRAME_LEN=28800` 的 640x360 RGB565 链路一致。
...
```sh
sudo ./pango_pcie_drm_c --no-display --width 640 --height 360 --line-bytes 1280 --frames 1 --dump-lines 8 --dump-frame /tmp/hdmi_pcie_8lines.rgb565
...
DRM/KMS 会自动选择 connected connector，优先输入尺寸对应模式；如果没有该模式，选择 preferred mode。显示 buffer 为 XRGB8888 dumb buffer，程序会把 640x360 RGB565 近邻缩放到实际 HDMI OUT 模式。
...
- 确认 FPGA 每行输出为 1280 字节，即 80 个 128-bit beat。
- 调整 `--delay-loops`。
...
本工程默认 FPGA 侧 bitstream 已正确完成 HDMI IN/图像输入、RGB565 组织、PCIe endpoint 枚举、DMA 请求响应和每行 1280 字节输出。RK3568 侧无法替代 FPGA 侧时序、帧同步和数据格式保证。
