`timescale 1ns/1ps
module burst_readwrite_processor
#
(
...
generate
	if (WRITE_DATA_BITS == 16)begin: Gen_WFIFO_16i
		afifo_16i_128o_512 dl_write_buf_16i(
			.wr_clk						(write_clk				),
...
generate
	...
	if (READ_DATA_BITS == 128)begin: Gen_RFIFO_128o
		afifo_128i_128o_512 dl_read_buf_128o (
			.wr_clk					(mem_clk					),                    // input
			.wr_rst					(read_fifo_aclr				),                    // input
			.wr_en					(rd_burst_data_valid		),                    // input
...
always @(posedge read_clk or posedge rst) begin
	...
	else if (rfifo_rdusedw[11:0] >= 12'd240 ) begin
		read_line_full <= 1'b1;
	end
	else begin
		read_line_full <= 1'b0;
